...
namespace Zadatak2.ProsliZadatak
{
	public interface ITodoRepository
	{
		...
		TodoItem Get(Guid todoId, String userId);
		/// <summary >
		...
		/// data</param >
		void Update(TodoItem todoItem, String userId);
		/// <summary >
		...
		/// </summary >
		List<TodoItem> GetCompleted(String userId);
		/// <summary >
		/// Gets all TodoItem objects in database for user that apply to the filter
		/// </summary >
		List<TodoItem> GetFiltered(Func<TodoItem, bool> filterFunction, String
		userId);

		List<TodoItemLabel> GetLabels(TodoItem item);
	}
}
